...
function run(build_info, run_info)
    ...
    if run_info.flags.uefi then
        ...
        table.insert(qemu_args,
            "if=pflash,format=raw,readonly=on,file=deps/edk2-ovmf/ovmf-code-" ..
            (build_info.target_architecture or "x86_64") .. ".fd,readonly=on")
    end
    ...
    elseif run_info.flags.hvf then
        ...
        if build_info.target_architecture == "x86_64" then
            table.insert(qemu_args,
                "Skylake-Client,x2apic=" .. (run_info.flags.x2apic and "on" or "off") .. ",+xsave,+avx")
