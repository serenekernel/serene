...
void acpi_init(void) {
    ...
    if(number_of_tables > 255) {
        assert(false && "Too many ACPI tables");
    }
    ...
    for(size_t i = 0; i < number_of_tables; i++) {
        uintptr_t entry_addr = (xsdp->revision >= 2) ? ((acpi_xsdt_t*) TO_HHDM(xsdp->xsdt_address))->entries[i] : ((acpi_rsdt_t*) TO_HHDM(xsdp->__rsdt_address))->entries[i];
