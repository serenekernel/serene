...
syscall_ret_t syscall_sys_process_create_empty() {
    ...
    handle_t handle = handle_create(HANDLE_TYPE_PROCESS, current_thread->thread_common.process->pid, HANDLE_CAPS_PROCESS_MAP | HANDLE_CAPS_PROCESS_COPY | HANDLE_CAPS_PROCESS_START | HANDLE_CAPS_PROCESS_DESTROY, (void*) new_process);
...
syscall_ret_t syscall_sys_start(uint64_t process_handle_value, uint64_t entry) {
    ...
    thread_t* thread = sched_thread_user_init(target_process->address_space, (virt_addr_t) entry);
