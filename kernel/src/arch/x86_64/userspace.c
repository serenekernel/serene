...
syscall_err_t dispatch_syscall(syscall_nr_t syscall_nr, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5) {
    ...
    if(syscall_nr >= 256) {
        printf("[systrace] %d:%d - %s(0x%llx, 0x%llx, 0x%llx, 0x%llx, 0x%llx) = %s\n", thread->thread_common.process->pid, thread->thread_common.tid, convert_syscall_number(syscall_nr), arg1, arg2, arg3, arg4, arg5, convert_syscall_error(SYSCALL_INVALID_SYSCALL));
    ...
    switch(entry.num_params) {
        case 0: {
            ...
            printf("[systrace] %d:%d - %s() = %s\n", thread->thread_common.process->pid, thread->thread_common.tid, convert_syscall_number(syscall_nr), convert_syscall_error(x));
            return x;
        }
        case 1: {
            syscall_err_t x = entry.handlers.handler1(arg1);
            printf("[systrace] %d:%d - %s(0x%llx) = %s\n", thread->thread_common.process->pid, thread->thread_common.tid, convert_syscall_number(syscall_nr), arg1, convert_syscall_error(x));
            return x;
        }
        case 2: {
            syscall_err_t x = entry.handlers.handler2(arg1, arg2);
            printf("[systrace] %d:%d - %s(0x%llx, 0x%llx) = %s\n", thread->thread_common.process->pid, thread->thread_common.tid, convert_syscall_number(syscall_nr), arg1, arg2, convert_syscall_error(x));
            return x;
        }
        case 3: {
            syscall_err_t x = entry.handlers.handler3(arg1, arg2, arg3);
            printf("[systrace] %d:%d - %s(0x%llx, 0x%llx, 0x%llx) = %s\n", thread->thread_common.process->pid, thread->thread_common.tid, convert_syscall_number(syscall_nr), arg1, arg2, arg3, convert_syscall_error(x));
            return x;
        }
        case 4: {
            syscall_err_t x = entry.handlers.handler4(arg1, arg2, arg3, arg4);
            printf("[systrace] %d:%d - %s(0x%llx, 0x%llx, 0x%llx, 0x%llx) = %s\n", thread->thread_common.process->pid, thread->thread_common.tid, convert_syscall_number(syscall_nr), arg1, arg2, arg3, arg4, convert_syscall_error(x));
            return x;
        }
        case 5: {
            syscall_err_t x = entry.handlers.handler5(arg1, arg2, arg3, arg4, arg5);
            printf("[systrace] %d:%d - %s(0x%llx, 0x%llx, 0x%llx, 0x%llx, 0x%llx) = %s\n", thread->thread_common.process->pid, thread->thread_common.tid, convert_syscall_number(syscall_nr), arg1, arg2, arg3, arg4, arg5, convert_syscall_error(x));
            return x;
        }
        default:
...
void userspace_init() {
    ...
    syscall_table[SYS_EXIT].num_params = 0;
    syscall_table[SYS_EXIT].handlers.handler0 = syscall_sys_exit;
