...
void fpu_init_bsp() {
    ...
    fpu_init_ap();
    if(__cpuid_get_feature_value(CPUID_FEATURE_XSAVE)) {
...
void fpu_init_ap() {
    assert(__cpuid_get_feature_value(CPUID_FEATURE_FXSR) && "FXSR not supported on this CPU!");
    // sse & fpu enable
    ...
    if(__cpuid_get_feature_value(CPUID_FEATURE_XSAVE)) {
        ...
        xcr0 |= 1 << 0; // XCR0.X87
        xcr0 |= 1 << 1; // XCR0.SSE
        if(__cpuid_get_feature_value(CPUID_FEATURE_AVX)) {
            printf("avx supported\n");
            xcr0 |= 1 << 2; // XCR0.AVX
        }
        if(__cpuid_get_feature_value(CPUID_FEATURE_AVX512)) {
